...
def get_metadata_at_coords_singlesite(nd2, **coords):
    import pims_nd2
    h = pims_nd2.ND2SDK

    _coords = {'t': 0, 'c': 0, 'z': 0, 'o': 0}
    _coords.update(coords)
    print(_coords['t'])
    c_coords = h.LIMUINT_4(int(_coords['t']), 
                           int(_coords['z']), int(_coords['o']))
    i = h.Lim_GetSeqIndexFromCoords(nd2._lim_experiment,
                                c_coords)
...
def extract_nd2_metadata_singlesite(f, interpolate=True, progress=None):
    """Interpolation fills in timestamps linearly for each well; x,y,z positions 
    are copied from the first time point. 
    """
    with ND2_Reader(f) as nd2:
        ...
        arr = []
        for t in progress(list(product(ts))):
            # boundaries = [0, nd2.sizes['m'] - 1]
            # skip = m not in boundaries and t > 0
            # if interpolate and skip:
            #     metadata = {}
            # else:
            metadata = get_metadata_at_coords_singlesite(nd2, t=t)
            metadata['t'] = t
            metadata['file'] = f
            arr += [metadata]
